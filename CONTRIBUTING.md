# Quench.NET - Contributing <!-- omit in toc -->
...
requests are welcome on [GitHub](https://github.com/synesissoftware/Quench.NET).


## Reporting issues

Use [GitHub Issues](https://github.com/synesissoftware/Quench.NET/issues). Include:

* **Quench.NET** version (NuGet package version or release tag);
...
3. Build and test:
   ...
   ```bash
   dotnet build Quench.NET.sln
   dotnet test Quench.NET.sln
   ```
...
* Builds treat warnings as errors; nullable reference types are enabled;
* Match existing naming, layout, and brace style in the file you edit;
* Public API in **`src/Quench.NET/`** must have XML documentation (`///`);
...
3. **Capitalisation**: the first word of a comment should be capitalised,
   unless it is a well-known name (for example, `Quench`, `NuGet`);
...
* XML documentation on affected public members in **`src/Quench.NET/`**;
